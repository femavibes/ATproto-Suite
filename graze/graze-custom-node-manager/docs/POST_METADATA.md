...
**langs** - Array of language codes ["en", "es"]
**$type** - Record type (app.bsky.feed.post)

### Bridged Content Fields
**tags** - Array of hashtags/categories from original platform (e.g., ["spotify", "Real Estate"])
**tags[*]** - Individual tag from original platform (without #)
**bridgyOriginalText** - Original text from bridged platform (Mastodon, web, etc.)
**bridgyOriginalUrl** - Original URL from bridged platform

**Note:** These fields only appear on posts bridged from other platforms via Bridgy Fed. The `tags` field contains hashtags or RSS categories from the source platform, separate from Bluesky's native `facets` hashtags.

...
class ListManager {
  ...
  async createList(locationId, bucketNumber = 1) {
    ...
    const listName = bucketNumber === 1 ? loc.name : `${loc.name}_${bucketNumber}`;
    ...
    const listUrl = `https://bsky.app/profile/${this.agent.session.did}/lists/${response.data.rkey}`;
    
    await this.pool.query(
      'INSERT INTO bluesky_lists (location_id, list_url, list_rkey, bucket_number) VALUES ($1, $2, $3, $4)',
      [locationId, listUrl, response.data.rkey, bucketNumber]
    );
    
    console.log(`Created list: ${listName} (${listUrl})`);
    await new Promise(resolve => setTimeout(resolve, this.rateLimitDelay));
    return { listUrl, rkey: response.data.rkey };
  }

  async addUserToList(did, locationId) {
    // Check for available list (under 3000 members)
  ...
  async removeUserFromList(did, locationId) {
    ...
    for (const list of lists.rows) {
      try {
        const listItems = await this.agent.com.atproto.repo.listRecords({
          repo: this.agent.session.did,
          collection: 'app.bsky.graph.listitem',
          limit: 100
        });
        
        const itemToDelete = listItems.data.records.find(item => 
